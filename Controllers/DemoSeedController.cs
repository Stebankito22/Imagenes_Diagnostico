...
public class DemoSeedController : ControllerBase
{
    ...
    public ActionResult SeedData([FromQuery] bool force = false)
    {
        try
        {
            Console.WriteLine($"[DemoSeed] Seed requested, force={force}");
            ...
            if (force && _context.TiposEstudio.Any())
            {
                ...
                Console.WriteLine("[DemoSeed] Existing data cleared");
            }
            ...
            _context.SaveChanges();
            Console.WriteLine($"[DemoSeed] TiposEstudio saved: {tipos.Count}");

            _context.Equipos.AddRange(equipos);
            _context.TecnicosEjecutores.AddRange(tecnicos);
            _context.SaveChanges();
            Console.WriteLine($"[DemoSeed] Equipos: {equipos.Count}, Tecnicos: {tecnicos.Count}");

            _context.OrdenesImagen.AddRange(ordenes);
            _context.SaveChanges();
            Console.WriteLine($"[DemoSeed] Ordenes: {ordenes.Count}");
            ...
            var estudios = new List<EstudioRealizado>
            {
                ...
                new() { Codigo = "EST-2026-002", OrdenImagenId = 2, TipoEstudioId = 4, TecnicoEjecutorId = 2, EquipoId = 3, FechaHoraInicio = DateTime.UtcNow.AddDays(-5).AddHours(10), FechaHoraFin = DateTime.UtcNow.AddDays(-5).AddHours(10.5), DosisRadiacion = 3.20m, ContrasteAplicado = false, EstadoInforme = "Completado" },
                new() { Codigo = "EST-2026-003", OrdenImagenId = 3, TipoEstudioId = 9, TecnicoEjecutorId = 4, EquipoId = 5, FechaHoraInicio = DateTime.UtcNow.AddDays(-4).AddHours(11), FechaHoraFin = DateTime.UtcNow.AddDays(-4).AddHours(11.5), DosisRadiacion = 0, ContrasteAplicado = false, EstadoInforme = "Completado" },
                new() { Codigo = "EST-2026-004", OrdenImagenId = 4, TipoEstudioId = 6, TecnicoEjecutorId = 2, EquipoId = 3, FechaHoraInicio = DateTime.UtcNow.AddDays(-3).AddHours(14), FechaHoraFin = DateTime.UtcNow.AddDays(-3).AddHours(14.75), DosisRadiacion = 8.50m, ContrasteAplicado = true, EstadoInforme = "Completado" },
                new() { Codigo = "EST-2026-005", OrdenImagenId = 5, TipoEstudioId = 7, TecnicoEjecutorId = 3, EquipoId = 4, FechaHoraInicio = DateTime.UtcNow.AddDays(-2).AddHours(8), FechaHoraFin = DateTime.UtcNow.AddDays(-2).AddHours(8.5), DosisRadiacion = 0, ContrasteAplicado = false, EstadoInforme = "Pendiente" },
                new() { Codigo = "EST-2026-006", OrdenImagenId = 6, TipoEstudioId = 5, TecnicoEjecutorId = 2, EquipoId = 3, FechaHoraInicio = DateTime.UtcNow.AddDays(-2).AddHours(10), FechaHoraFin = DateTime.UtcNow.AddDays(-2).AddHours(10.75), DosisRadiacion = 6.80m, ContrasteAplicado = true, EstadoInforme = "Completado" },
                new() { Codigo = "EST-2026-007", OrdenImagenId = 7, TipoEstudioId = 11, TecnicoEjecutorId = 4, EquipoId = 5, FechaHoraInicio = DateTime.UtcNow.AddDays(-1).AddHours(9), FechaHoraFin = DateTime.UtcNow.AddDays(-1).AddHours(9.5), DosisRadiacion = 0, ContrasteAplicado = false, EstadoInforme = "Pendiente" },
                new() { Codigo = "EST-2026-008", OrdenImagenId = 8, TipoEstudioId = 12, TecnicoEjecutorId = 6, EquipoId = 7, FechaHoraInicio = DateTime.UtcNow.AddDays(-1).AddHours(11), FechaHoraFin = DateTime.UtcNow.AddDays(-1).AddHours(11.33), DosisRadiacion = 0.35m, ContrasteAplicado = false, EstadoInforme = "Completado" },
                new() { Codigo = "EST-2026-009", OrdenImagenId = 9, TipoEstudioId = 1, TecnicoEjecutorId = 5, EquipoId = 2, FechaHoraInicio = DateTime.UtcNow.AddHours(-10), FechaHoraFin = DateTime.UtcNow.AddHours(-9.75), DosisRadiacion = 0.10m, ContrasteAplicado = false, EstadoInforme = "Pendiente" },
                new() { Codigo = "EST-2026-010", OrdenImagenId = 10, TipoEstudioId = 8, TecnicoEjecutorId = 3, EquipoId = 4, FechaHoraInicio = DateTime.UtcNow.AddHours(-4), FechaHoraFin = DateTime.UtcNow.AddHours(-3.5), DosisRadiacion = 0, ContrasteAplicado = false, EstadoInforme = "Pendiente" },
            };

            _context.EstudiosRealizados.AddRange(estudios);
            _context.SaveChanges();
            Console.WriteLine($"[DemoSeed] Estudios: {estudios.Count}");

            var informes = new List<InformeRadiologico>
            {
                new() { Codigo = "INF-2026-001", EstudioRealizadoId = 1, Radiologo = "Dr. Fernando Mamani", Hallazgos = "Campos pulmonares limpios. Silueta cardíaca de tamaño normal.", Diagnostico = "Radiografía de tórax sin hallazgos patológicos.", Observaciones = "", FechaEmision = DateTime.UtcNow.AddDays(-6) },
                new() { Codigo = "INF-2026-002", EstudioRealizadoId = 2, Radiologo = "Dra. Patricia Soto", Hallazgos = "Sin evidencia de lesiones ocupantes de espacio.", Diagnostico = "TAC de cráneo normal.", Observaciones = "", FechaEmision = DateTime.UtcNow.AddDays(-4) },
                new() { Codigo = "INF-2026-003", EstudioRealizadoId = 3, Radiologo = "Dr. Fernando Mamani", Hallazgos = "Hígado de tamaño y ecogenicidad normales.", Diagnostico = "Ecografía abdominal normal.", Observaciones = "", FechaEmision = DateTime.UtcNow.AddDays(-3) },
                new() { Codigo = "INF-2026-004", EstudioRealizadoId = 4, Radiologo = "Dra. Patricia Soto", Hallazgos = "Nódulo pulmonar en lóbulo superior derecho de 12mm.", Diagnostico = "Nódulo pulmonar sospechoso.", Observaciones = "", FechaEmision = DateTime.UtcNow.AddDays(-2) },
                new() { Codigo = "INF-2026-005", EstudioRealizadoId = 6, Radiologo = "Dr. Fernando Mamani", Hallazgos = "Realce adecuado con contraste.", Diagnostico = "TAC de tórax con hallazgos mínimos.", Observaciones = "", FechaEmision = DateTime.UtcNow.AddDays(-1) },
                new() { Codigo = "INF-2026-006", EstudioRealizadoId = 8, Radiologo = "Dra. Carmen Ríos", Hallazgos = "Tejido fibroglandular heterogéneo. BIRADS 1.", Diagnostico = "Mamografía BIRADS 1 - Negativa.", Observaciones = "", FechaEmision = DateTime.UtcNow.AddHours(-12) },
            };

            _context.InformesRadiologicos.AddRange(informes);
            _context.SaveChanges();
            Console.WriteLine($"[DemoSeed] Informes: {informes.Count}");

            Console.WriteLine("[DemoSeed] All data saved successfully");
            return Ok(new
            {
        ...
        {
            Console.WriteLine($"[DemoSeed] ERROR: {ex.Message}");
            Console.WriteLine($"[DemoSeed] Stack: {ex.StackTrace}");
